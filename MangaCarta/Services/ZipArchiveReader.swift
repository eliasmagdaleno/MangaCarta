...
public struct ZipArchiveReader: Sendable {
        ...
        case 8:
            ...
            let decoded = result.withUnsafeMutableBytes { destination in
                compressed.withUnsafeBytes { source in
                    compression_decode_buffer(destination.bindMemory(to: UInt8.self).baseAddress!, Int(entry.uncompressedSize),
                                               source.bindMemory(to: UInt8.self).baseAddress!, Int(entry.compressedSize), nil, COMPRESSION_ZLIB)
                }
            }
    ...
    public func chapters() throws -> [Chapter] {
        let pages = try PageSelector.chapters(entries: entries) { try self.data(for: $0) }
        return pages
    }
    ...
    private static func parse(data: Data, limits: Limits) throws -> [Entry] {
        ...
        if data.count >= 22 { for i in stride(from: data.count - 22, through: min, by: -1) where data[i] == 0x50 && data[i + 1] == 0x4b && data[i + 2] == 0x05 && data[i + 3] == 0x06 { eocd = i; break } }
        ...
        guard count <= limits.maxEntryCount, offset + size <= UInt64(e) else { throw ZipArchiveError.truncated }
    ...
    private static func validatePath(_ name: String) throws {
        if name.hasPrefix("/") || name.hasPrefix("\\") || name.split(whereSeparator: { $0 == "/" || $0 == "\\" }).contains("..") { throw ZipArchiveError.pathTraversal(name) }
    }
}
