...
- **Index and scripts.** The index document is validated by §4 exactly as a fetched one. Its URL
  and its `script` references are absolute `https://` URLs on the host **`bundled.invalid`**
  (RFC 2606: reserved, never resolvable), so they satisfy §4's absolute-HTTPS rule with no
  bundled-only exception in the validator. *(Amended 2026-09-22: this bullet said bundle-relative;
  a relative or custom-scheme URL fails §4, and an earlier draft that used one for the index only
  let the script fetch escape to the network.)* `scriptSHA256` is still checked against the
  resource bytes — the digest is a tamper and packaging check here, not a trust one.
- **Transport.** The app's transport routes on the host: a `bundled.invalid` URL, index *or*
  script, is read from the bundle and never reaches the network; any other host goes to the
  network transport. `fetchIndex` never returns `.movedPermanently` for a bundled URL. The size
  bounds in §10 apply.
...
- **Records (§8).** The repository record's URL is the constant `bundled.invalid` index URL, which
  does not change between installs, so there is nothing to re-resolve. The bundle's file path is
  looked up by the transport at fetch time, never stored. *(Amended 2026-09-22, with the bullet
  above.)* Otherwise the three records are as §8.1.
